...
export function defaultSuites(messages = 500): CorpusSuite[] {
  return [
    { route: "GET /v1/candles", schema: CandleResponse, corpus: candlesCorpus(messages, 0xc9) },
    { route: "GET /v1/devices", schema: DeviceResponse, corpus: devicesCorpus(messages, 0xd9) },
    { route: "GET /v1/feed", schema: FeedResponse, corpus: feedCorpus(messages, 0xf9) },
    { route: "GET /v1/events", schema: EventResponse, corpus: eventsCorpus(messages, 0xe9) },
    { route: "GET /v1/orders/:id", schema: OrderResponse, corpus: ordersCorpus(messages, 0x09) },
  ];
}
...
export function runProfileSuite(suites: CorpusSuite[] = defaultSuites()): CorpusResult[] {
  ...
  console.log("  route              msgs   json   gzip     br4    col   prof   full   dictionary");
  for (const r of results) {
    const cell = (v: number) => String(v).padStart(6);
    console.log(
      `  ${r.route.padEnd(17)} ${String(r.messages).padStart(4)} ${cell(r.json)} ${cell(r.gzip)} ` +
        `${cell(r.brotli)} ${cell(r.columnar)} ${cell(r.profiled)} ${cell(r.full)}   ` +
